...
(function () {
  ...
  var pins = window.pins.createPinsArray(pinFragment);
  ...
  */
  var showAd = function (pinNode) {
    ...
    filtersContainer.before(window.card.createAd(window.card.ads[index]));
  ...
  var activatePage = function (isActive) {
    if (!isActive) {
      ...
      pins = window.pins.createPinsArray(pinFragment);
