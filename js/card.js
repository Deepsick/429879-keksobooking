...
(function () {
  ...
  var createAd = function (author) {
    ...
    newAd.querySelector('.popup__type').textContent = window.data.adTypeToPopupType[author.offer.type];
    ...
    for (var i = 0; i < author.offer.photos.length; i++) {
    ...
    if (author.offer.features.length > 0) {
      for (var j = 0; j < author.offer.features.length; j++) {
        var featuresItem = document.createElement('li');
        featuresItem.classList.add('popup__feature');
        featuresItem.classList.add('popup__feature--' + author.offer.features[j]);
